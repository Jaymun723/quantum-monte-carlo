...
class Worldline:  # "w"
    ...
    def generate_valid_state(self, spins, i, j0, set_spins):  # with prints
        ...
        if i == 0 and spins[i, j0] == 0:
            ...
            set_spins[j0] = 1
            chosen_value = np.random.choice([-1, 1])
            ...
            if validity:
                ...
                return self.generate_valid_state(spins, 0, j0, set_spins)
        ...
        if i == m - 1:
            ...
            if spins[(i + 1) % m, j0] == 0 and spins[(i + 1) % m, neighbor] == 0:
                ...
                chosen_spin = np.random.choice([j0, neighbor])
                new_spins = spins.copy()
        ...
        if parity == 0:
            ...
            chosen_spin = np.random.choice([neighbor, j0])
            new_spins = spins.copy()
            new_spins[(i + 1) % m, chosen_spin] = spins[i, j0]
            new_set_spins = set_spins.copy()
